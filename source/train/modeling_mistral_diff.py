...
class MistralDifferentialAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".

    Refer: https://github.com/microsoft/unilm/blob/master/Diff-Transformer/multihead_diffattn.py
    """
    ...
    def __init__(self, config: MistralConfig, layer_idx: Optional[int] = None):
        ...
        # print("depth:",self.layer_idx+1)
        self.lambda_init = lambda_init_fn(self.layer_idx) # depth:0~L
        ...
        self.subln = MistralRMSNorm(2 * self.head_dim, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        # print("attn_weights[:,:,1]",attn_weights[:,:,1])
...
MISTRAL_ATTENTION_CLASSES = {
    "eager": MistralDifferentialAttention,
...
# TODO(joao): add me back asap :)
class MistralDifferentialDecoderLayer(nn.Module):
...
class MistralDifferentialPreTrainedModel(PreTrainedModel):
    config_class = MistralConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["MistralDifferentialDecoderLayer"]
...
class MistralDifferentialModel(MistralDifferentialPreTrainedModel):
    """
    ...
    def __init__(self, config: MistralConfig):
        ...
        self.layers = nn.ModuleList(
            [MistralDifferentialDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class MistralDifferentialForCausalLM(MistralDifferentialPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MistralDifferentialModel(config)
        self.vocab_size = config.vocab_size
